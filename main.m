...
x1 = rand*x_container*0.1 + x_container*0.45;
y1 = rand*y_container*0.1 + y_container*0.45;
z1 = 0.0002;
...
%                     end
        ...
        if length(intersect) == 0
            ...
            y1 = intersect(indx, 2);
            z1 = intersect(indx, 3);
            selfinx = intersect(indx, 4);
            theta = rand*2*pi;
            phi = rand*pi;
            x2 = x(1, selfinx) + r(1, selfinx)*sin(phi)*cos(theta);
            y2 = y(1, selfinx) + r(1, selfinx)*sin(phi)*sin(theta);
            z2 = z(1, selfinx) + r(1, selfinx)*cos(phi);
            intensity = intensity*0.6;
        end
    end
    depth(j, 1) = 0.00015 - z1;
end